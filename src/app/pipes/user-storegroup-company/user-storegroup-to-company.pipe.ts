...
@Pipe({
  name: 'userStoreGroupToCompany'
})
export class UserStoreGroupToCompanyPipe implements PipeTransform {
  ...
  transform(userRoles: UserRoleModel[]): string {
    ...
    var allItems=[...new Set(userRoles.filter(x => x.group != null ).map(x => x.group.name))];
